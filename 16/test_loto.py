...
class TestProgram(unittest.TestCase):
    ...
    # def test_initial_state(self):
        ...
        # #self.assertEqual(len(self.game.cards), 3)  # Проверяем количество карточек
        # self.assertEqual(self.new_game.list_players[0].name, 'Alex')  # Проверяем количество карточек
        # #self.assertTrue(all(card.is_empty() for card in self.game.cards))  # Проверяем, что все карточки пусты
        #self.assertTrue(all(card.is_empty() for card in self.game.cards))  # Проверяем, что все карточки пусты
